...
def create_flete(
    ...
    if repositories.get_flete_by(
        db, data.remitente_id, data.producto_id, data.tipo_carga_id, data.numero_factura
    ):
        raise HTTPException(
            status_code=409,
            detail="El Flete para este remitente con este producto ya existe",
        )
...
def edit_flete(
    ...
    if (
        ...
        exists = repositories.get_flete_by(
            db,
            data.remitente_id,
            data.producto_id,
            data.tipo_carga_id,
...
def get_flete_reports(db: Session) -> str:
    ...
    title_cell.value = "Número de Factura"
    title_cell.font = Font(bold=True)

    title_cell = ws.cell(row=1, column=6)
    title_cell.value = "Número de CRT"
    title_cell.font = Font(bold=True)
    ...
    title_cell = ws.cell(row=1, column=38)
    ...
    for row, item in enumerate(datalist):
        ...
        value_cell.value = item.numero_factura
        ...
        value_cell.value = item.tipo_flete
