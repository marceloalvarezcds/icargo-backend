...
def create_contacto(
    ...
    repositories.create_propietario_contacto_gestor_carga(
        db,
        data.cargo,
        propietario,
        contacto,
...
def edit_contacto(
    ...
    propietario_contacto_obj = repositories.get_propietario_contacto_gestor_carga_by(
        db, propietario.id, contacto.id, gestor_carga_id
    )
    if propietario_contacto_obj:
        repositories.edit_propietario_contacto_gestor_carga(
            propietario_contacto_obj,
            db,
            cargo,
            propietario,
            contacto,
        ...
        create_contacto(db, data, propietario, gestor_carga_id, modified_by)
    return contacto
...
def delete_contacto(
    ...
    propietario_contacto_obj = (
        repositories.get_propietario_contacto_gestor_carga_by_cargo_id(
            db, cargo_id, propietario.id, contacto.id, gestor_carga_id
        )
    )
    if propietario_contacto_obj:
        repositories.delete_propietario_contacto_gestor_carga(
            db,
            propietario_contacto_obj.id,
            modified_by,
        )
    return contacto


def update_propietario_contacto_list(
    db: Session,
    ...
    if gestor_carga_id:
        contactos: List[PropietarioContactoGestorCarga] = propietario.contactos
        ...
        if has_contactos:
            ...
            for contactoForm in contactoList:
                create_contacto(
                    db, contactoForm, propietario, gestor_carga_id, modified_by
                )
