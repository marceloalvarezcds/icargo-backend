...
def get_orden_carga_anticipo_retirado_pdf_by_id(db: Session, id: int) -> str:
    ...
    # Generar un PDF simple para prueba
    try:
        simple_html = "<html><body><h1>Test PDF</h1></body></html>"
        simple_pdf_filename = os.path.join(REPORTS_FOLDER, f"test_{id}.pdf")
        logger.info('Generating simple PDF...')
        from_string(simple_html, simple_pdf_filename, {"page-size": "Legal"})
        logger.info(f'Simple PDF generated: {simple_pdf_filename}')
    except Exception as e:
        logger.error(f"Error generating simple PDF: {e}")
        raise HTTPException(status_code=500, detail="Error generating simple PDF")

    return f"test_{id}.pdf"
