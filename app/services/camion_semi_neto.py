...
def get_camion_semi_neto_list_by_producto_id(
    ...
    camion_semi_neto_list.extend(
        repositories.get_camion_semi_neto_list_by_producto_id_null(db, gestor_carga_id)
    )
...
def get_camion_semi_neto_list_by_camion_id_and_producto_id(
    ...
    camion_semi_neto_list.extend(
        repositories.get_camion_semi_neto_list_by_camion_id_and_producto_id_null(
...
def check_if_combination_exists(
    ...
    exists = None
    msg = "Ya existe esta combinación"
    if producto_id:
        msg = f"La combinación de Camion Nº {camion_id} con Semi Nº {semi_id}, Producto Nº {producto_id} y neto {number_format(neto)} ya existe"  # noqa: B950
        exists = repositories.get_camion_semi_neto_by_camion_id_and_semi_id_and_producto_id_and_neto(  # noqa: B950
            db, camion_id, semi_id, producto_id, neto, gestor_carga_id
        )
    else:
        msg = f"La combinación de Camion Nº {camion_id} con Semi Nº {semi_id} y neto {number_format(neto)} ya existe"  # noqa: B950
        exists = repositories.get_camion_semi_neto_by_camion_id_and_semi_id_and_neto(
