...
from app.models import ComposicionJuridica, GestorCarga, Remitente, TipoDocumento
...
from .gestor_carga_remitente_seeds import gestor_carga_remitente_seeds
from .remitente_contacto_gestor_carga_seeds import remitente_contacto_gestor_carga_seeds
...
def remitente_seeds(
    ...
    if tipo_documento and composicion_juridica and ciudad_id:
        obj = get_remitente_by(db, tipo_documento.id, numero_documento)
        if not obj:
            remitente = Remitente(
                nombre=nombre,
                nombre_corto=None,
            ...
            db.add(remitente)
            db.commit()
            gestor_carga_remitente_seeds(db, remitente, gestor_carga, alias)
            cargo = get_cargo_by_descripcion(db, cargo_descripcion)
            contacto = get_contacto_by_email(db, contacto_email)
            remitente_contacto_gestor_carga_seeds(
                db, cargo, remitente, contacto, gestor_carga, contacto_alias
            )
